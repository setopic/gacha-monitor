...
docs/
  ...
  00-meta/              規約とテンプレート（グラフの語彙そのもの）
    graph-rules.md      ルール ID G001〜G020 と直し方
...
**`G019` はエラー**（切れた行は既にただの文字列として出ている）。
しきい値と必須の節は `tools/graph/schema.py` にある。

**`G018` と `G019` だけは「見た目」を見ている。** グラフの整合性は全部通るのに
GitHub 上の表示だけが壊れる、という種類の問題が実際に起きたため。
...
見る。`G020` は `status` を問わず**本文のリンク**を見て、取り下げた決定を
断りなく引いていないかを確かめる。**同じ段落で置き換え先も指していれば黙る。**
