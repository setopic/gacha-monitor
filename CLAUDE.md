...
**原則（決定の梯子と、簡略化しても削らないもの）は
[META-06](docs/00-meta/principles.md) にある。毎回そちらを読む。**

**実装に着手する前と、差分を出す前に `/lean` を回す**（`.claude/skills/lean/`）。
`/grill` が決めた「何を作るか」に対して、どれだけ少なく作るかを決める。
